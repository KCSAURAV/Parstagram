  ...
  func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
    ...
    Parse.initialize(
      with: ParseClientConfiguration(block: { (configuration: ParseMutableClientConfiguration) -> Void in
        ...
        configuration.server = "https://aqueous-thicket-30224.herokuapp.com/parse"
      })
    )
    ...
    if PFUser.current() != nil{
      ...
      let FeedNC = main.instantiateViewController(withIdentifier: "FeedNC") // Create a VC (bluprint) to instantiate one of our VC
      
      // Now we have our instance of that Navigation VC
      // 1 per application that display the screen 1 at a time
      window?.rootViewController = FeedNC
      // Root VC is the one being displayed
    }
